type t = {
  a_interval: IntervalD.t;
  a_mod8: Mod8D.t;
  a_limset: LimSetD.t;
}

let top =
  { a_interval = IntervalD.top;
    a_mod8 = Mod8D.top;
    a_limset = LimSetD.top;
  }
...
let ole a b =
  IntervalD.ole a.a_interval b.a_interval &&
  Mod8D.ole a.a_mod8 b.a_mod8 &&
  LimSetD.ole a.a_limset b.a_limset

let pp fmt a = Format.fprintf fmt "(%a, %a, %a)" IntervalD.pp a.a_interval Mod8D.pp a.a_mod8 LimSetD.pp a.a_limset
...
let try_concretize (a: t) (limit: int): DS.Int64Set.t option =
  let s1 = IntervalD.try_concretize a.a_interval (limit * 8) in
  let s2 = LimSetD.try_concretize a.a_limset (limit * 8) in
  let ns = match (s1, s2) with
    | (None, None) -> None
    | (Some s1, None) -> Some s1
    | (None, Some s2) -> Some s2
    | (Some s1, Some s2) -> Some (DS.Int64Set.union s1 s2)
  in
  match ns |> Option.map (DS.Int64Set.filter (fun x -> Mod8D.contains a.a_mod8 x)) with
  | None -> None
  | Some s -> if DS.Int64Set.cardinal s > limit then None else Some s