...
| Unique u -> (match (find_opt (MemRef.UniqueR u) a) with
...
| Register r -> (match (find_opt (MemRef.RegisterR r) a) with
...
let process_load (p: Prog.t) (a: t) (d: OctagonD.t) (pointerv: VarNode.t) (outv: VarNode.t) =
  match pointerv.varNode_node, MemRef.convert_varnode outv with
  | Unique u, Some outmr ->
    (match Option.bind (find_opt (MemRef.UniqueR u) a) (fun x -> AbsVal.try_concretize x 20) with
    | Some vset -> add outmr (AbsVal.of_limset (LimSetD.LimSet (Int64Set.map (fun x -> Prog.get_rom p x outv.varNode_width) vset))) a
    | None -> 
    add outmr (AbsVal.of_interval (OctagonD.request_interval d (MemRef.UniqueR u))) a)
...
| Abop (_, _, _) -> na
| Auop (Uint_sext, vn) -> let v = (eval_varnode a d vn) in
