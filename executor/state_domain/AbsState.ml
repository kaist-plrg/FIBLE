...
include TripleD.MakeJoinSemiLatticeWithTop_Record (NonRelStateD) (OctagonD) (BoolPowerD) (struct
  type t = __
...
let post_contour_single (p: Prog.t) (ls: Loc.t) (lf: Loc.t) (a: t)(i: Inst.t): t = 
  if (ls = (0x100753L, 0)) then Format.printf "astate: %a inst: %a\n" pp a Inst.pp i else ();
  match i with
  ...
  | Icbranch (condv, truelocv) -> (match truelocv with
  |  {
    varNode_node = Ram trueloc ;
    varNode_width = _ } ->
      (match condv.varNode_node with
            | Unique i -> (match (BoolPowerD.find_opt (MemRef.UniqueR i) a.value_boolpower) with
              | Some b ->
...
let try_concretize_vn (a: t) (vn: VarNode.t) (limit: int): Int64Set.t option = 
  ...
  | Unique i -> (match (NonRelStateD.find_opt (MemRef.UniqueR i) a.value_nonrel) with
    | Some a -> AbsVal.try_concretize a limit
    | None -> None)
  | Register r -> (match (NonRelStateD.find_opt (MemRef.RegisterR r) a.value_nonrel) with
    | Some a -> AbsVal.try_concretize a limit
    | None -> None)
  | _ -> None