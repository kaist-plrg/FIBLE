...
include TripleD.MakeJoinSemiLattice_Record(ContourD)(JumpD)(FSAbsD)(
  struct
    type t = __
...
type flow_heurstic_type = Prog.t -> Loc.t -> AbsState.t -> Inst.t -> heuristic_result

let gen_bb_single (h: flow_heurstic_type) (p: Prog.t) (l: Loc.t) (a: AbsState.t) (i: Inst.t): (Loc.t option * Loc.t option) =
...
let gen_ujump_single (h: flow_heurstic_type) (p: Prog.t) (l: Loc.t) (a: AbsState.t) (i: Inst.t): LocSetD.t option =
  match h p l a i with
  | HrUnsound s -> Some s
  | HrSound s -> None
  | HrExit -> None
...
let gen_jump_single (h: flow_heurstic_type) (p: Prog.t) (l: Loc.t) (a: AbsState.t) (i: Inst.t): LocSetD.t =
  match h p l a i with
...
let flow_heuristic_simple (p: Prog.t) (l: Loc.t) (a: AbsState.t) (i: Inst.t): heuristic_result = match i with
| Ijump (Jcall, vn) -> HrFallthrough
| Ijump (_, vn) -> (match vn with
  |  {
    varNode_node = Ram a ;
    varNode_width = _ } -> HrSound (LocSetD.singleton (a, 0))
...
| Ijump_ind (_, vn) -> (match AbsState.try_concretize_vn a vn 20 with
  | Some a -> HrSound (LocSetD.of_seq ((Int64Set.to_seq a) |> Seq.map (fun a -> (a, 0))))
  | None -> HrUnsound (LocSetD.empty)
  )
| Icbranch (cn, jn) -> (match jn with
  |  {
    varNode_node = Ram a ;
    varNode_width = _ } -> HrSound (LocSetD.of_list ((a, 0) :: (fallthru p l) :: []))
  | _ -> HrUnsound (LocSetD.empty))
| Iunimplemented -> HrUnsound (LocSetD.empty)
| _ -> HrSound (LocSetD.singleton (fallthru p l))

let gen_contour (p: Prog.t) (a: FSAbsD.t): (UJumpD.t * BoundaryPointD.t * JumpD.t) =
  let u = FSAbsD.mapi (fun l a -> match Prog.get_ins p l with
    | Some i -> gen_ujump_single flow_heuristic_simple p l a i
    | None -> None
  ) a in
  let b = FSAbsD.fold (fun l a (bens, bexs) ->
    let ins = Prog.get_ins p l in
    match ins with
    | Some i -> let (eno, exo) = gen_bb_single flow_heuristic_simple p l a i in
                (Option.fold ~none:bens ~some:(fun l -> LocSetD.add l bens) eno, Option.fold ~none:bexs ~some:(fun l -> LocSetD.add l bexs) exo)
    | None -> (bens, bexs)
  ) a (LocSetD.empty, LocSetD.empty) in
  let j = FSAbsD.mapi (fun l a -> match Prog.get_ins p l with
...
let post_contour_fs (p: Prog.t) (c: ContourD.t) (sj: JumpD.t) (a: FSAbsD.t): FSAbsD.t =
  let locHash = LocHashtbl.create 100 in
  FSAbsD.iter (fun ls s ->
  ...
  LocSetD.iter (fun l -> LocHashtbl.update locHash l (fun v -> Some AbsState.top)
  ) (fst c.boundary_point);
  LocHashtbl.to_seq locHash |> FSAbsD.of_seq 

let post (p: Prog.t) (c: t): t =
  let (u, b, j) = gen_contour p (c.abs_state) in
  let (nu, nb, nj) = (UJumpD.join c.analysis_contour.unsound_jump u,
                      (fun (a, b) (c, d) -> (LocSetD.join a c, LocSetD.join b d)) c.analysis_contour.boundary_point b,
                      JumpD.join c.sound_jump j) in
...
let init (p: Prog.t) (l: Loc.t): t =
  { analysis_contour = {unsound_jump = UJumpD.empty; boundary_point = (LocSetD.singleton l, LocSetD.empty)};
