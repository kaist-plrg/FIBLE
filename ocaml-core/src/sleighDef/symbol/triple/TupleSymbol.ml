...
type ('varnode_t, 'tuple_t) poly_t = ('varnode_t, 'tuple_t) TypeDef.tuple_poly_t
type ptr_t = TypeDef.tuple_ptr_t
type t = TypeDef.tuple_t

let of_purevalue (v : PureValueSymbol.t) : ('varnode_t, 'triple_t) poly_t =
  Family (FamilySymbol.of_purevalue v)

let of_purevalue_fix (v : PureValueSymbol.t) : t =
  Family (FamilySymbol.of_purevalue v)

let of_context (v : ContextSymbol.t) : ('varnode_t, 'triple_t) poly_t =
  Family (FamilySymbol.of_context v)

let of_context_fix (v : ContextSymbol.t) : t =
  Family (FamilySymbol.of_context v)

let of_name (v : NameSymbol.t) : ('varnode_t, 'triple_t) poly_t =
  Family (FamilySymbol.of_name v)

let of_name_fix (v : NameSymbol.t) : t = Family (FamilySymbol.of_name v)

let of_valuemap (v : ValueMapSymbol.t) : ('varnode_t, 'triple_t) poly_t =
