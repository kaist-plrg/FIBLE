...
let join a b =
  ...
  | Top, _ -> Top
  | _, Top -> Top
  | LimSet a, LimSet b -> lift_set (Int64Set.union a b)

let meet a b =
  match (a, b) with
  | Top, a -> a
  | a, Top -> a
  | LimSet a, LimSet b -> lift_set (Int64Set.inter a b)

let le a b =
  match (a, b) with
  | _, Top -> true
  | Top, _ -> false
  | LimSet a, LimSet b -> Int64Set.subset a b
...
let sext (a : t) (in_width : int32) (out_width : int32) : t =
  match a with
  | Top -> Top
  | LimSet a ->
      Int64Set.of_seq
...
let try_concretize (x : t) (limit : int) : Int64Set.t option =
  match x with
  | Top -> None
  | LimSet a -> if Int64Set.cardinal a > limit then None else Some a
