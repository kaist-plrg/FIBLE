...
type t =
  | Iload of (VarNode.t * VarNode.t * RegId.t)
  | Istore of (VarNode.t * VarNode.t * VarNode.t)
  | Isload of (Const.t * RegId.t)
  | Isstore of (Const.t * VarNode.t)
  | Iassignment of (Assignable.t * RegId.t)
  | INop
...
let pp (fmt : Format.formatter) (p : t) =
  match p with
  | Iload (i0, i1, o) ->
      Format.fprintf fmt "%a = *[%a]%a;" RegId.pp o VarNode.pp i0 VarNode.pp i1
  | Istore (i0, i1, i2) ->
  ...
  | Iassignment (i, o) ->
      Format.fprintf fmt "%a = %a;" RegId.pp o Assignable.pp i
