...
module Immutable = struct
  ...
  let parse_cmd (raw_line : string option) : repl_cmd =
    ...
    | None -> Quit
    | Some s ->
        Scanf.sscanf_opt s "q%!" Quit
        |> Option.fold ~none:(Scanf.sscanf_opt s "s%!" Step) ~some:Option.some
        |> Option.fold
             ~none:(Scanf.sscanf_opt s "show %r%!" Loc.scan (fun l -> Show l))
             ~some:Option.some
        |> Option.fold
             ~none:(Scanf.sscanf_opt s "rlist%!" Reachable)
             ~some:Option.some
        |> Option.fold
             ~none:(Scanf.sscanf_opt s "c%!" Continue)
             ~some:Option.some
        |> Option.fold
             ~none:(Scanf.sscanf_opt s "blist%!" BList)
             ~some:Option.some
        |> Option.fold
             ~none:(Scanf.sscanf_opt s "badd %r%!" Loc.scan (fun l -> BAdd l))
             ~some:Option.some
        |> Option.fold
             ~none:(Scanf.sscanf_opt s "bremove %d%!" (fun n -> BRemove n))
             ~some:Option.some
        |> Option.value ~default:Ignore

  let rec repl (p : Prog.t) (c : t) (ls : Loc.t List.t) (bs : Loc.t List.t)
      (continue : Bool.t) : Unit.t =
    let cmd =
