...
let dump_flag =
  { cfa = ref false; l1 = ref false; basic_block = ref false; spfa = ref false; l2 = ref false }

let speclist =
  [
    ("-i", Arg.Set_string ifile, ": input file");
    ("-g", Arg.Set_string ghidra_path, ": ghidra path");
...
let instHash : (int * L0.Inst.t list) Int64Hashtbl.t = Int64Hashtbl.create 1000

let instfunc (si : SpaceInfo.t) (fd : Unix.file_descr) (addr : int64) :
    (int * L0.Inst.t list) option =
  if Int64Hashtbl.mem instHash addr then Some (Int64Hashtbl.find instHash addr)
    ...
    if inst_len = 0l then None
      ...
      List.iter
        (fun x -> print_endline (Format.asprintf "%a" L0.Inst.pp x))
        pcodes;
...
let dump_cfa (cfa_res: (String.t * Addr.t * Cfa.CFA.Immutable.t) list) (dump_path: string) =
  List.iter
  (fun (fname, _, x) ->
    let dump_cfa_path =
      Filename.concat dump_path (fname ^ ".boundary")
    in
...
let dump_spfa (spfa_res: (L1.Func.t * Spfa.SPFA.Immutable.t) list) (dump_path: string) =
  List.iter
  (fun ((func: L1.Func.t), x) ->
    let fname: String.t = Option.value func.nameo ~default:(Format.asprintf "%a" Loc.pp func.entry) in
    let dump_spfa_path =
    ...
    match accesses with
     ...
     | Spfa.AccessD.Fin (accesses) -> (match Int64SetD.min_elt_opt accesses with
...
let () =
  ...
  if !ifile = "" then raise (Arg.Bad "No input file")
    ...
    List.iter (fun x -> print_endline (Format.sprintf "func %s" x)) target_funcs;
    let func_with_addrs =
    ...
    in
    let l1 : L1.Prog.t = Cfa.L0toL1.translate_prog_from_cfa l0 cfa_res in
    let spfa_res : (L1.Func.t * Spfa.SPFA.Immutable.t) list =
      l1.funcs |> List.map (fun x -> (x, Spfa.SPFA.Immutable.analyze x 32L))
    in
    let l2 : L2.Prog.t = Spfa.L1toL2.translate_prog_from_spfa l1 spfa_res 32L in
    if
    ...
    if !(dump_flag.l1) then
      L1.Prog.dump_prog l1 !dump_path ((Filename.basename !ifile) |> Filename.remove_extension)
    else ();
    if !(dump_flag.basic_block) then
      L1.Prog.dump_basic_block l1 !dump_path ((Filename.basename !ifile) |> Filename.remove_extension)
    else ();
    if !(dump_flag.spfa) then dump_spfa spfa_res !dump_path else ();
    if !(dump_flag.l2) then
      L2.Prog.dump_prog l2 !dump_path (((Filename.basename !ifile) |> Filename.remove_extension))
    else ();
