...
module PCode_Raw = struct
  ...
  let pp fmt (p : t) =
    Format.fprintf fmt "{opcode=%ld; inputs=%a; output=%a}" p.opcode
...
let pcode_raw_to_pcode (si : SpaceInfo.t) (p : PCode_Raw.t) : L0.Inst.t =
  ...
  in
  let mkJIump a = L0.Inst.Ijump_ind (a, inputs 0) in
