...
let of_const (x : Int64.t) = { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat x }

let bop_const bop x y width = match (x, y) with
...
let uop_const uop x = match x with
  | { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat x } ->
    { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat (uop x) }
  | _ -> top

let add x y width = match (x, y) with
  | { have_sp = FlatBoolD.Flat true; offset = FlatInt64D.Flat x },
    { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat y } ->
      { have_sp = FlatBoolD.Flat true; offset = FlatInt64D.Flat (Int64.add x y) }
  | { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat x },
    { have_sp = FlatBoolD.Flat true; offset = FlatInt64D.Flat y } ->
    { have_sp = FlatBoolD.Flat true; offset = FlatInt64D.Flat (Int64.add x y) }
  | { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat x },
  { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat y } ->
  { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat (Int64.add x y) }
  | _ -> top

let sub x y width = match (x, y) with
| { have_sp = FlatBoolD.Flat true; offset = FlatInt64D.Flat x },
{ have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat y } ->
  { have_sp = FlatBoolD.Flat true; offset = FlatInt64D.Flat (Int64.sub x y) }
| { have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat x },
{ have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat y } ->
{ have_sp = FlatBoolD.Flat false; offset = FlatInt64D.Flat (Int64.sub x y) }
| _ -> top

let mul x y width = bop_const Int64.mul x y width
let sext x inwidth outwidth = uop_const (fun x -> Int64Ext.sext x inwidth outwidth) x